 ...
 * own DOM: any collapsed seat whose subtree contains our `.hq-sch` card has its
 * `hidden` attribute removed, kept removed by a MutationObserver across DSH's
 * virtualized re-renders. Scoped to our cards only — other tools still collapse
 * normally. No DSH source is touched.
 */
 ...
 * for every collapsed seat that wraps our `.hq-sch` card, remove the attribute.
 * A rAF-coalesced MutationObserver reasserts this whenever DSH's virtualized
 * renderer re-adds it. Scoped to our cards — other tools keep collapsing.
 * Returns a disposer.
 *
...
export function installSchematicUncollapser(): () => void {
  ...
  const revealSeat = (seat: HTMLElement): void => {
    if (seat.querySelector('.hq-sch')) seat.removeAttribute('hidden')
  }
  let scheduled = false
  const sweep = (): void => {
    scheduled = false
