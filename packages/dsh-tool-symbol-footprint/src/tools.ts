...
async function resolveToken(auth: HuaqiuAuthService['auth']): Promise<string | null> {
  ...
  try {
    ...
    log.warn({ error: String((err as Error)?.message || err) }, 'could not resolve the Huaqiu access token')
    return null
  }
}
...
async function handleDirectFootprint(
  ...
  try {
    ...
    log.warn({ error: String((err as Error)?.message || err) }, 'direct-footprint confirmation unavailable')
    return Object.assign(
      { status: 'generated', kind: 'footprint', autoGenerated: true },
      generated,
      {
