...
/** Unified DSH-plugin logger for the artifacts plugin (via @hqedge/logging).
 *  Lazy — see dsh-auth/src/host.ts for the rationale (mocked DSH-home tests). */
let pluginLog: Logger | null = null
function getPluginLog(): Logger {
  if (pluginLog === null) pluginLog = createLogger({ component: 'dsh-artifacts' })
  return pluginLog
}

/**
 * Minimal, dependency-free facade every internal call site uses. Delegates to
 * `@hqedge/logging` so plugin diagnostics land in the HQ Edge log tree
 * (`<HQ_EDGE_HOME>/logs/dsh-artifacts/`) — preserved verbatim across deps
...
export function log(level: 'debug' | 'warn', msg: string, extra?: Record<string, unknown>): void {
  ...
  if (level === 'warn') l.warn(extra ?? {}, msg)
  else l.debug(extra ?? {}, msg)
}
